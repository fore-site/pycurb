...
def assert_result(
    result, *, allowed=True, remaining=4, reset_at=RESET_AT, limit, rule_name
):
...
class TestAsyncAlgorithms:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                token_bucket_module,
                LimitRule(
                    name="tb", algorithm="token_bucket", capacity=7, refill_rate=2.5
                ),
                "token_bucket",
                {
                    "key": "client:1",
            ...
            (
                ...
                leaky_bucket_module,
                LimitRule(
                    name="lb", algorithm="leaky_bucket", capacity=3, leak_rate=0.75
                ),
                "leaky_bucket",
    ...
    async def test_check_calls_expected_storage_method_and_wraps_result(
        ...
        assert storage.calls == [
            (storage_method, _prefixed_expected(expected_kwargs, rule))
        ]
        assert_result(
            result,
            limit=expected_kwargs.get("limit", expected_kwargs.get("capacity")),
            rule_name=rule.name,
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                sliding_window_module,
                LimitRule(
                    name="sw_denied", algorithm="sliding_window", limit=2, window=60
                ),
                2,
            ),
            (
                ...
                fixed_window_module,
                LimitRule(
                    name="fw_denied", algorithm="fixed_window", limit=2, window=60
                ),
                2,
            ),
            (
            ...
            (
                ...
                leaky_bucket_module,
                LimitRule(
                    name="lb_denied", algorithm="leaky_bucket", capacity=2, leak_rate=1
                ),
                2,
            ),
        ],
    )
    async def test_denied_storage_decision_is_preserved(
        self, monkeypatch, algorithm, module, rule, expected_limit
    ):
        freeze_time(monkeypatch, module)
    ...
    async def test_token_bucket_uses_limit_as_capacity_fallback(self, monkeypatch):
        ...
        rule = LimitRule(
            name="tb_limit", algorithm="token_bucket", limit=12, refill_rate=3
        )
    ...
    @pytest.mark.asyncio
    async def test_token_bucket_derives_refill_rate_from_capacity_and_window(
        self, monkeypatch
    ):
        freeze_time(monkeypatch, token_bucket_module)
        rule = LimitRule(
            name="tb_window", algorithm="token_bucket", capacity=20, window=4
        )
        storage = AsyncSpyStorage()
        ...
        assert storage.calls == [
            (
                "token_bucket",
                {
                    "key": f"{rule.name}:client:tb-window",
                    "capacity": 20,
                    "refill_rate": 5,
                    "now": BASE_TIME,
                },
    ...
    @pytest.mark.asyncio
    async def test_token_bucket_prefers_capacity_and_explicit_refill_rate(
        self, monkeypatch
    ):
        freeze_time(monkeypatch, token_bucket_module)
        rule = LimitRule(
        ...
        result = await AsyncTokenBucketAlgorithm().check(
            "client:tb-precedence", rule, storage
        ...
        assert storage.calls == [
            (
                "token_bucket",
                {
                    "key": f"{rule.name}:client:tb-precedence",
                    "capacity": 20,
                    "refill_rate": 1.5,
                    "now": BASE_TIME,
                },
    ...
    @pytest.mark.asyncio
    async def test_leaky_bucket_derives_leak_rate_from_capacity_and_window(
        self, monkeypatch
    ):
        freeze_time(monkeypatch, leaky_bucket_module)
        rule = LimitRule(
            name="lb_window", algorithm="leaky_bucket", capacity=20, window=4
        )
        storage = AsyncSpyStorage()
        ...
        assert storage.calls == [
            (
                "leaky_bucket",
                {
                    "key": f"{rule.name}:client:lb-window",
                    "capacity": 20,
                    "leak_rate": 5,
                    "now": BASE_TIME,
                },
    ...
    @pytest.mark.asyncio
    async def test_leaky_bucket_prefers_capacity_and_explicit_leak_rate(
        self, monkeypatch
    ):
        freeze_time(monkeypatch, leaky_bucket_module)
        rule = LimitRule(
        ...
        result = await AsyncLeakyBucketAlgorithm().check(
            "client:lb-precedence", rule, storage
        )  # type: ignore[arg-type]
        ...
        assert storage.calls == [
            (
                "leaky_bucket",
                {
                    "key": f"{rule.name}:client:lb-precedence",
                    "capacity": 20,
                    "leak_rate": 1.5,
                    "now": BASE_TIME,
                },
    ...
    async def test_leaky_bucket_uses_limit_as_capacity_fallback(self, monkeypatch):
        freeze_time(monkeypatch, leaky_bucket_module)
        rule = LimitRule(
            name="lb_limit", algorithm="leaky_bucket", limit=9, leak_rate=3
        )
        storage = AsyncSpyStorage()
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                AsyncSlidingWindowAlgorithm(),
                LimitRule.model_construct(
                    name="bad_sw", algorithm="sliding_window", limit=None, window=60
                ),
                "limit",
            ),
            (
                AsyncSlidingWindowAlgorithm(),
                LimitRule.model_construct(
                    name="bad_sw", algorithm="sliding_window", limit=10, window=None
                ),
                "window",
            ),
            (
                AsyncFixedWindowAlgorithm(),
                LimitRule.model_construct(
                    name="bad_fw", algorithm="fixed_window", limit=None, window=60
                ),
                "limit",
            ),
            (
                AsyncFixedWindowAlgorithm(),
                LimitRule.model_construct(
                    name="bad_fw", algorithm="fixed_window", limit=10, window=None
                ),
                "window",
            ),
            (
            ...
            (
                AsyncTokenBucketAlgorithm(),
                LimitRule.model_construct(
                    name="bad_tb", algorithm="token_bucket", capacity=10, refill_rate=0
                ),
                "positive",
            ),
            (
            ...
            (
                AsyncLeakyBucketAlgorithm(),
                LimitRule.model_construct(
                    name="bad_lb", algorithm="leaky_bucket", capacity=10, leak_rate=None
                ),
                "leak_rate",
            ),
            (
                AsyncLeakyBucketAlgorithm(),
                LimitRule.model_construct(
                    name="bad_lb", algorithm="leaky_bucket", capacity=10, leak_rate=0
                ),
                "positive",
            ),
            (
            ...
            (
                AsyncGcraAlgorithm(),
                LimitRule.model_construct(
                    name="bad_gcra", algorithm="gcra", capacity=10, refill_rate=None
                ),
                "refill_rate",
            ),
            (
                AsyncGcraAlgorithm(),
                LimitRule.model_construct(
                    name="bad_gcra", algorithm="gcra", capacity=10, refill_rate=0
                ),
                "positive",
            ),
        ],
    )
    async def test_invalid_rule_shapes_raise_clear_value_error(
        self, algorithm, rule, message
    ):
...
class TestSyncAlgorithms:
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                LimitRule(
                    name="sw_sync", algorithm="sliding_window", limit=5, window=60
                ),
                "sliding_window",
                {"key": "client:1", "limit": 5, "window": 60, "now": BASE_TIME},
            ),
            (
                ...
                LimitRule(
                    name="fw_sync", algorithm="fixed_window", limit=10, window=30
                ),
                "fixed_window",
                {"key": "client:1", "limit": 10, "window": 30, "now": BASE_TIME},
            ),
            ...
            (
                ...
                LimitRule(
                    name="lb_sync", algorithm="leaky_bucket", capacity=3, leak_rate=0.75
                ),
                "leaky_bucket",
                {"key": "client:1", "capacity": 3, "leak_rate": 0.75, "now": BASE_TIME},
            ),
            (
                ...
                gcra_sync_module,
                LimitRule(
                    name="gcra_sync", algorithm="gcra", capacity=7, refill_rate=2.5
                ),
                "gcra",
                {"key": "client:1", "capacity": 7, "rate": 2.5, "now": BASE_TIME},
            ),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                fixed_window_sync_module,
                LimitRule(
                    name="fw_denied_sync", algorithm="fixed_window", limit=2, window=60
                ),
                2,
            ),
            (
    ...
    def test_denied_storage_decision_is_preserved(
        self, monkeypatch, algorithm, module, rule, expected_limit
    ):
        freeze_time(monkeypatch, module)
    ...
    def test_token_bucket_uses_limit_as_capacity_fallback(self, monkeypatch):
        ...
        rule = LimitRule(
            name="tb_limit_sync", algorithm="token_bucket", limit=12, refill_rate=3
        )
    ...
    def test_gcra_uses_limit_as_capacity_fallback_sync(self, monkeypatch):
        ...
        rule = LimitRule(
            name="gcra_limit_sync", algorithm="gcra", limit=12, refill_rate=3
        )
        storage = SyncSpyStorage()
    ...
    def test_token_bucket_derives_refill_rate_from_capacity_and_window(
        self, monkeypatch
    ):
        freeze_time(monkeypatch, token_bucket_sync_module)
        rule = LimitRule(
            name="tb_window_sync", algorithm="token_bucket", capacity=20, window=4
        )
        storage = SyncSpyStorage()
    ...
    def test_leaky_bucket_derives_leak_rate_from_capacity_and_window(self, monkeypatch):
        ...
        rule = LimitRule(
            name="lb_window_sync", algorithm="leaky_bucket", capacity=20, window=4
        )
        storage = SyncSpyStorage()
    ...
    def test_leaky_bucket_uses_limit_as_capacity_fallback(self, monkeypatch):
        freeze_time(monkeypatch, leaky_bucket_sync_module)
        rule = LimitRule(
            name="lb_limit_sync", algorithm="leaky_bucket", limit=9, leak_rate=3
        )
        storage = SyncSpyStorage()
    ...
    def test_invalid_rule_shapes_raise_clear_value_error(
        self, algorithm, rule, message
    ):
        with pytest.raises(ValueError, match=message):
