...
class TestAsyncRateLimitDecorator:
    ...
    async def test_named_rule_with_key_extractor(self):
        ...
        if isinstance(resolver, AsyncRuleResolver):
            await resolver.add_rule(
                LimitRule(name="test", algorithm="fixed_window", limit=2, window=10)
            )
    ...
    async def test_inline_rule_gcra_algorithm(self):
        ...
        @rate_limit(
            ...
            limit_str="2/10s",
            algorithm="gcra",
            key_extractor=arg_extractor("uid"),
        )
        async def func(uid: str):
            return "ok"
    ...
    async def test_named_rule_gcra_with_key_extractor(self):
        ...
        if isinstance(resolver, AsyncRuleResolver):
            await resolver.add_rule(
                LimitRule(
                    name="gcra_test", algorithm="gcra", capacity=2, refill_rate=0.2
                )
            )

        @rate_limit(
            limiter, rule_name="gcra_test", key_extractor=lambda user: str(user)
        )
...
class TestSyncRateLimitDecorator:
    def test_named_rule(self):
        ...
        if isinstance(resolver, RuleResolver):
            resolver.add_rule(
                LimitRule(name="test", algorithm="fixed_window", limit=1, window=10)
            )
    ...
    async def test_named_multiple_rules_decorator_async(self):
        ...
        if isinstance(resolver, AsyncRuleResolver):
            await resolver.add_rule(
                LimitRule(name="r1", algorithm="fixed_window", limit=2, window=10)
            )
            await resolver.add_rule(
                LimitRule(name="r2", algorithm="fixed_window", limit=3, window=10)
            )

        @rate_limit(
            limiter, rule_name=["r1", "r2"], key_extractor=lambda user: str(user)
        )
        async def fn(user: str):
            return "ok"
    ...
    async def test_decorator_multiple_rules_missing_raises_async(self):
        ...
        if isinstance(resolver, AsyncRuleResolver):
            await resolver.add_rule(
                LimitRule(name="exists", algorithm="fixed_window", limit=1, window=10)
            )
    ...
    def test_named_multiple_rules_decorator_sync(self):
        ...
        if isinstance(resolver, RuleResolver):
            resolver.add_rule(
                LimitRule(name="r1", algorithm="fixed_window", limit=1, window=10)
            )
            resolver.add_rule(
                LimitRule(name="r2", algorithm="fixed_window", limit=2, window=10)
            )
