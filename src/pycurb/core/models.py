...
class LimitRule(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, description="Short unique identifier for the rule."
    )
    algorithm: Literal[
        "sliding_window", "fixed_window", "token_bucket", "leaky_bucket", "gcra"
    ] = Field(..., description="Rate limit algorithm to use.")

    # Window algorithms require limit and window; token bucket can use them if capacity and refill_rate are not provided.
    limit: Optional[int] = Field(
        default=None,
        gt=0,
        description="Maximum requests allowed (window-based) or capacity (token bucket/gcra).",
    )
    window: Optional[int] = Field(
        default=None,
        gt=0,
        description="Time window in seconds (for window-based algorithms) or base to calculate refill rate.",
    )
    ...
    capacity: Optional[int] = Field(
        default=None,
        gt=0,
        description="Maximum capacity for token bucket or gcra (burst limit).",
    )

    refill_rate: Optional[float] = Field(
        default=None,
        gt=0,
        description="Tokens added per second or requests allowed per second for GCRA.",
    )

    # Leaky bucket parameter
    leak_rate: Optional[float] = Field(
        default=None, gt=0, description="Requests processed per second."
    )

    # Metadata for extra conditional logic
    metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="Arbitrary user data for extra conditional logic.",
    )
...
class RateLimitResult(BaseModel):
    ...
    remaining: int = Field(
        ..., ge=0, description="Remaining requests/tokens in current window/bucket."
    )
    reset_at: float = Field(
        ..., description="Unix timestamp (seconds) when the limit resets."
    )
    limit: int = Field(
        ..., gt=0, description="The configured limit (for X-RateLimit-Limit header)."
    )
    retry_after: Optional[int] = Field(
        default=None,
        ge=0,
        description="Seconds to wait before retrying (only when allowed is False).",
    )
